...
public class CloudWatchLogsClientEmitter implements LogEventEmitter<CloudWatchLogsClient> {
   ...
   */
  public CloudWatchLogsClientEmitter(String logGroupName, String logStreamName, String awsRegion) {
  ...
  @Override
  public CloudWatchLogsClient getEmitter() {
  ...
  public void emit(Map<String, Object> logEvent) {
    try {
      ...
      logger.severe(String.format("Failed to process log event: %s", error.getMessage()));
      throw error;
    }
  }
  ...
  private void createLogGroupIfNeeded() {
    try {
      ...
      this.getEmitter().createLogGroup(request);
      logger.info(String.format("Created log group: %s", this.logGroupName));
    } catch (ResourceAlreadyExistsException e) {
      logger.fine(String.format("Log group %s already exists", this.logGroupName));
    } catch (AwsServiceException e) {
      logger.severe(
          String.format("Failed to create log group %s: %s", this.logGroupName, e.getMessage()));
      throw e;
    }
  }
  ...
  private void createLogStreamIfNeeded() {
    try {
      ...
      this.getEmitter().createLogStream(request);
      logger.info(String.format("Created log stream: %s", this.logStreamName));
    } catch (ResourceAlreadyExistsException e) {
      logger.fine(String.format("Log stream %s already exists", this.logStreamName));
    } catch (AwsServiceException e) {
      logger.severe(
          String.format("Failed to create log stream %s: %s", this.logStreamName, e.getMessage()));
      throw e;
    }
  }
  ...
  private void sendLogBatch(List<InputLogEvent> batch) {
    ...
    try {
      this.getEmitter().putLogEvents(request);
      ...
      logger.fine(
          String.format(
              "Successfully sent %d log events (%d KB) in %d ms",
              batch.size(), batchSizeKB, elapsedMs));
      ...
      try {
        ...
        logger.fine(
            String.format(
                "Successfully sent %d log events (%d KB) in %d ms after creating resources",
                batch.size(), batchSizeKB, elapsedMs));

      } catch (AwsServiceException retryError) {
        logger.severe(
            String.format(
                "Failed to send log events after creating resources: %s", retryError.getMessage()));
        throw retryError;
      }
    } catch (AwsServiceException e) {
      logger.severe(String.format("Failed to send log events: %s", e.getMessage()));
      throw e;
    }
  }
  ...
  private boolean isValidLogEvent(Map<String, Object> logEvent) {
    ...
    if (messageSize > CW_MAX_EVENT_PAYLOAD_BYTES) {
      logger.warning(
          String.format(
              "Log event size %d exceeds maximum allowed size %d. Truncating.",
              messageSize, CW_MAX_EVENT_PAYLOAD_BYTES));
      int maxMessageSize =
    ...
    if (timeDiff > CW_EVENT_TIMESTAMP_LIMIT_PAST
      ...
      logger.severe(
          String.format(
              "Log event timestamp %d is either older than 14 days or more than 2 hours in the future. Current time: %d",
